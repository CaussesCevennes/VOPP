...
function OPP(providers, theme) {
  ...
  var setupMap = function(){
    ...
    self.photoMap1 = L.map('photo1', {zoomControl:false, crs: photoCRS, center: [0, 0], zoom: 0, zoomSnap: self.phZoomSnap, maxBoundsViscosity: 0.75});
    self.photoMap2 = L.map('photo2', {zoomControl:false, crs: photoCRS, center: [0, 0], zoom: 0, zoomSnap: self.phZoomSnap, maxBoundsViscosity: 0.75});
  ...
  var updatePhotos = function (fit){
    ...
    if (self.viewMode != 'SINGLE' && !(hasActiveSketch1 || hasActiveSketch2)){
      let m1 = photoLay1.photo['MATRIX'][photoLay2.photo['DATE']];
      let m2 = photoLay2.photo['MATRIX'][photoLay1.photo['DATE']];
      if (m1){
        $('#chkAlign1').show();
        if ($('#chkAlign1>input').prop('checked')){
          photoLay1.matrix = m1;
        }
      } else {
        $('#chkAlign1').hide();
        console.log(`No transformation matrix from ${photoLay1.photo['DATE']} to ${photoLay2.photo['DATE']}`);
      }
      if (m2){
        $('#chkAlign2').show();
        if ($('#chkAlign2>input').prop('checked')){
          photoLay2.matrix = m2;
        }
      } else {
        $('#chkAlign2').hide();
        console.log(`No transformation matrix from ${photoLay2.photo['DATE']} to ${photoLay1.photo['DATE']}`);
      }
    } else {
      $('#chkAlign1, #chkAlign2').hide();
    }
    ...
    if (self.viewMode == 'SINGLE'){
      ...
      self.photoMap1.setMaxBounds(photoLay1.getBounds());//.pad(0.25));
    }
  ...
  var connectEvents = function (){
    ...
    $('.chkAlign>input').on('change', function() {
      updatePhotos(false);
    });
